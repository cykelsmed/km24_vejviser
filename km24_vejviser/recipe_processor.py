"""
Recipe Processor: Behandling og validering af LLM-genererede opskrifter.

Dette modul indeholder al logik til at:
- Transformere rå LLM-output til struktureret format
- Validere og berige opskrifter med API-data
- Generere pædagogisk indhold
- Anvende defaults og sikre datakvalitet
"""
...
def _get_default_sources_for_module(module_name: str) -> list[str]:
    ...
    elif "centraladministration" in module_lower:
        return [
            "Miljøministeriet",
...
async def _enrich_with_module_specific_filters(step: dict, goal: str) -> dict:
    ...
    try:
        ...
        module_name = (
            ...
            if isinstance(step.get("module"), dict)
            else step.get("module")
        )
        if not module_name:
            return step
        ...
        # Tilføj default sources for web-kildemoduler
        if module_card.requires_source_selection and (
            not step.get("source_selection")
            or len(step.get("source_selection", [])) == 0
        ):
            default_sources = _get_default_sources_for_module(module_name)
            if default_sources:
                step["source_selection"] = default_sources
                logger.info(
                    f"Tilføjet default kilder for {module_name}: {default_sources}"
                )

        return step
    except Exception as e:
        logger.error(f"Fejl i _enrich_with_module_specific_filters: {e}")
        return step
...
def validate_content_relevance(recipe: dict, goal: str) -> list[str]:
    ...
    for angle in recipe.get("potential_story_angles", []):
        ...
        if overlap < 2:  # Less than 2 keywords in common
            warnings.append(f"Potentielt irrelevant story angle: '{angle[:60]}...'")

    # Validate cross references mention actually used modules
    used_modules = {
        step.get("module", {}).get("name", "") for step in recipe.get("steps", [])
    }
    ...
    for xref in recipe.get("creative_cross_references", []):
        ...
        has_relevant_module = any(module in xref_lower for module in used_modules)
...
def validate_module_logic(recipe: dict) -> List[str]:
    ...
    if recipe.get("steps") and len(recipe["steps"]) > 0:
        ...
        is_identification = any(
            word in title_lower
            for word in ["identificer", "find", "kortlæg", "opbyg", "start med"]
        )

        if is_identification:
            # Check for wrong modules
            if module_name == "Status" and any(
                word in search_string.lower()
                for word in ["konkurs", "ophør", "likvidation"]
            ):
                warnings.append(
                    "KRITISK: Trin 1 bruger Status-modulet med konkurs-søgning til identifikation. "
                    "Dette finder KUN lukkede virksomheder. Brug i stedet Registrering med branchekoder."
                )
            ...
            if module_name != "Registrering":
                warnings.append(
                    "ANBEFALING: Trin 1 (identifikation) bør typisk bruge Registrering-modulet "
                    "med branchekoder for at finde ALLE relevante virksomheder."
                )
...
async def validate_and_clean_filters(step: dict) -> dict:
    ...
    for filter_key in filters.keys():
        ...
        # Web source filters
        if filter_key in ["source_selection", "kilder", "sources"]:
            if "web_source" not in supported_types:
                to_remove.append(filter_key)
                logger.warning(
                    f"Module {module_name} does not support source selection"
                )
...
def ensure_critical_filters(recipe: dict, goal: str) -> dict:
    ...
    logger.info(
        f"Detected industry keyword '{detected_keyword}' in goal → branch codes: {branch_codes}"
    )

    # Iterate through steps and fix Registrering steps
    for step in recipe.get("steps", []):
        module_name = step.get("module", {}).get("name", "")
        step.get("title", "").lower()
        ...
        if module_name == "Registrering":
            ...
            if not filters.get("Branche") or filters.get("Branche") == []:
                ...
                logger.info(
                    f"✓ Auto-added Branche filter {branch_codes} to Registrering step based on keyword '{detected_keyword}'"
                )
                ...
                if "rationale" in step:
                    step[
                        "rationale"
                    ] += f" (Auto-tilføjet branchekoder for {detected_keyword})"
...
def validate_step(step: dict, step_number: int) -> list[str]:
    ...
    if (
        "module" in step
        and "is_web_source" in step["module"]
        and step["module"]["is_web_source"]
    ):
...
def validate_module(module: dict, step_number: int) -> list[str]:
    ...
    if name not in official_modules and not any(
        official in name for official in official_modules.keys()
    ):
        errors.append(
            f"Trin {step_number}: Ugyldigt modulnavn '{name}'. Skal være et af de officielle moduler."
        )
...
def validate_search_syntax(search_string: str, step_number: int) -> list[str]:
    ...
    if "," in search_string:
        errors.append(
            f"Trin {step_number}: Brug semikolon ; i stedet for komma for parallelle variationer"
        )
...
def validate_filters(filters: dict, step_number: int) -> list[str]:
    ...
    if not found_categories:
        errors.append(
            f"Trin {step_number}: Filtre skal indeholde mindst én kategori (geografi, branche, beløb)"
        )
...
def validate_notification(notification: str, step_number: int) -> list[str]:
    ...
    if notification.lower() not in valid_notifications:
        errors.append(
            f"Trin {step_number}: Ugyldig notifikationskadence '{notification}'. Skal være: løbende, daglig, ugentlig, interval"
        )
...
def generate_context_block(goal: str, modules: list[dict], scope: dict) -> dict:
    ...
    if "Arbejdstilsyn" in modules_used:
        background.append(
            "Arbejdstilsynet registrerer over 15.000 reaktioner årligt på tværs af alle brancher"
        )

    if "Tinglysning" in modules_used:
        background.append(
            "Ejendomsdata kan afsløre økonomiske relationer og ejerskabsstrukturer"
        )

    # Specific expectations with numbers
    what_to_expect = []
    if "Registrering" in modules_used:
        what_to_expect.append(
            "5-20 nye virksomheder pr. måned i udvalgte brancher (afhænger af geografisk område)"
        )
    if "Status" in modules_used:
        what_to_expect.append("1-5 statusændringer pr. måned i målgruppen")
        what_to_expect.append(
            "Mønstre udvikler sig typisk over 3-12 måneder, ikke dage eller uger"
        )
    if "Arbejdstilsyn" in modules_used:
        what_to_expect.append(
            "2-10 tilsynsreaktioner pr. måned afhængig af brancher og geografi"
        )
    ...
    if "Arbejdstilsyn" in modules_used:
        caveats.append(
            "Myndighedskampagner kan skabe kunstige toppe - vurder tidslige mønstre kritisk"
        )
    if "Status" in modules_used:
        caveats.append(
            "Ikke alle sammenfald mellem reaktioner og statusændringer er kausale"
        )
    if any(m in modules_used for m in ["Registrering", "Personbogen"]):
        caveats.append(
            "Adressematch kræver manuel verifikation (forskellige formater i registre)"
        )
    ...
    if "Arbejdstilsyn" in modules_used:
        coverage_parts.append(
            "Arbejdstilsynsdata: Fra 2015 og frem, inkl. påbud og vejledninger"
        )
...
def generate_ai_assessment(recipe: dict, goal: str) -> dict:
    ...
    if (
        any(w in goal_lower for w in ["vognm", "transport", "social dumping"])
        and "arbejdstilsyn" in goal_lower
    ):
        focus = "Social dumping og arbejdsmiljø i transportbranchen"

    # Detail/konkurs
    elif any(w in goal_lower for w in ["butik", "detail"]) and any(
        w in goal_lower for w in ["konkurs", "lukk", "ophør"]
    ):
    ...
    # Count approximate monitors
    num_monitors = len(steps)
    scale_desc = f"{num_monitors} monitorer i {area}"

    # Expected pattern
    expected_signals = []
    if "Registrering" in modules_used and "Status" in modules_used:
        expected_signals.append("Mønstre vil udvikle sig gradvist over tid")
        expected_signals.append("Gentagne hændelser hos samme aktører")
    elif "Arbejdstilsyn" in modules_used:
        expected_signals.append("Alvorlige påbud hos problemvirksomheder")
        expected_signals.append("Gentagne kritikpunkter i samme branche")
    else:
        expected_signals.append("Data vil akkumulere over tid")

    return {
        "search_plan_summary": scale_desc,  # Summary of the search plan
        "scale": scale_desc,
        "focus": f"Fokus: {focus}",
...
def generate_hit_definition(step: dict, module_info: dict) -> dict:
    ...
    if module_name == "Registrering":
        ...
        if branch_codes:
            ...
            if len(branch_codes) > 3:
                branch_str = (
                    ", ".join(formatted_codes) + f" (+{len(branch_codes)-3} flere)"
                )
            else:
    ...
    return {"hit_types": hit_types, "indicators": indicators}
...
def generate_step_rationale(step: dict, goal: str, step_index: int) -> str:
    ...
    if step_index == 0:
        ...
        return rationales.get(
            module_name, f"{module_name} danner fundamentet for analysen."
        )
    ...
    if module_name == "Arbejdstilsyn":
        if step_index <= 1:  # Early in pipeline
            return (
                "Tilsynsreaktioner kan forklare efterfølgende statusændringer. "
                "Dokumentér tidspunkter for at spore kausal rækkefølge."
            )
        else:  # Later in pipeline
            return (
                "Tilsynsreaktioner kan forklare statusændringer set tidligere. "
                "Tjek om kritik forudgik konkurser."
            )

    return later_rationales.get(
        module_name,
        f"{module_name} tilføjer kontekst og verifikation til tidligere fund.",
    )
...
async def enrich_recipe_with_api(raw_recipe: dict) -> dict:
    ...
    for step in raw_recipe.get("investigation_steps", []):
        ...
        if not module_id:
            logger.warning(
                f"Module '{module_name}' not found in API, skipping step {step.get('step')}"
            )
            continue
        ...
        if not module_details_response.success:
            logger.warning(
                f"Could not fetch details for module {module_name}, using step as-is"
            )
        ...
        for filter_key, filter_values in raw_filters.items():
            ...
            if not matching_part:
                logger.warning(
                    f"Filter '{filter_key}' not found in {module_name} parts, skipping"
                )
                continue
            ...
            if filter_key_lower == "kommune":
                ...
                for value in filter_values:
                    if value.lower() in valid_municipalities:
                        ...
                        logger.warning(
                            f"Removed invalid municipality '{value}' for filter '{filter_key}' in module '{module_name}'"
                        )
                ...
                for value in filter_values:
                    if value in valid_branch_codes:
                        ...
                        logger.warning(
                            f"Removed invalid branch code '{value}' for filter '{filter_key}' in module '{module_name}'"
                        )
                ...
                for value in filter_values:
                    if value.lower() in valid_regions:
                        ...
                        logger.warning(
                            f"Removed invalid region '{value}' for filter '{filter_key}' in module '{module_name}'"
                        )

            # Handle generic_value parts - fetch and validate against API
            elif part_type == "generic_value" and part_id:
                try:
                    valid_values = filter_catalog.get_valid_generic_values_for_part(
                        part_id
                    )
                    if not valid_values:
                    ...
                    if valid_values:
                        for value in filter_values:
                            if value in valid_values:
                                ...
                                logger.warning(
                                    f"Removed invalid value '{value}' for filter '{filter_key}' in module '{module_name}'"
                                )
                    else:
                        validated_values = filter_values  # No validation data available
                except Exception as e:
                    logger.error(
                        f"Error validating generic values for {filter_key}: {e}"
                    )
            ...
            # Handle web_source parts - mark for manual selection
                ...
                if "strategic_note" not in step["details"]:
                    step["details"][
                        "strategic_note"
                    ] = f"PÅKRÆVET: Vælg konkrete kilder manuelt for {module_name}"
            ...
            if validated_values:
                ...
                logger.warning(
                    f"All values removed for filter '{filter_key}' in module '{module_name}', skipping filter"
                )

        # Add enriched step with validated filters and module_id
        enriched_steps.append(
            {**step, "filters": validated_filters, "module_id": module_id}
        )

    # Return enriched recipe
    return {**raw_recipe, "investigation_steps": enriched_steps}
...
def coerce_raw_to_target_shape(raw: dict, goal: str) -> dict:
    ...
    if goal:
        target["scope"]["primary_focus"] = (
            goal[:100] + "..." if len(goal) > 100 else goal
        )
        ...
        for i, step in enumerate(raw["investigation_steps"], 1):
            normalized_step = {
                ...
                "notification": _normalize_notification(
                    step.get("recommended_notification")
                    or step.get("details", {}).get("recommended_notification", "daily")
                ),
                "delivery": "email",
                "source_selection": step.get(
                    "source_selection",
                    step.get("details", {}).get("source_selection", []),
                ),
                "strategic_note": step.get("strategic_note")
                or step.get("details", {}).get("strategic_note"),
            ...
            normalized_step = _ensure_filters_before_search_string(
                normalized_step, goal
            )
            ...
            if normalized_step["search_string"] in ["søgning", "search", "søg", ""]:
                ...
                if normalized_step.get("filters", {}).get(
                    "virksomhed"
                ) or normalized_step.get("filters", {}).get("cvr"):
                    # Empty is fine when using company filter
                    normalized_step["search_string"] = ""
                    module_name = normalized_step.get("module", {}).get(
                        "name", "Unknown"
                    )
                    logger.info(
                        f"Cleared meaningless search string for {module_name} (using company filter)"
                    )
...
def apply_min_defaults(recipe: dict) -> None:
    ...
    if not recipe.get("overview", {}).get("strategy_summary"):
        recipe.setdefault("overview", {})[
            "strategy_summary"
        ] = "Systematic investigation using KM24 modules"
    if not recipe.get("overview", {}).get("creative_approach"):
        recipe.setdefault("overview", {})[
            "creative_approach"
        ] = "Data-driven approach with cross-referencing"
    ...
    if not recipe.get("notifications"):
        recipe["notifications"] = {"primary": "daily", "channels": ["email"]}
    ...
    for step in recipe.get("steps", []):
        ...
        if not step.get("search_string"):
            ...
            if search_in_filters:
                ...
                if isinstance(search_in_filters, list):
                    ...
                    step["search_string"] = (
                        " ".join(search_in_filters)
                        if len(search_in_filters) > 1
                        else (search_in_filters[0] if search_in_filters else "")
                    )
                else:
                ...
                module_name = (
                    step.get("module", {}).get("name", "Unknown")
                    if isinstance(step.get("module"), dict)
                    else step.get("module", "Unknown")
                )
                step["search_string"] = _get_default_search_string_for_module(
                    module_name
                )
                logger.info(
                    f"Genereret default søgestreng for {module_name}: {step['search_string']}"
                )
        else:
            ...
            step["search_string"] = _standardize_search_string(
                step["search_string"], module_name
            )
            logger.info(
                f"Standardiseret søgestreng for {module_name}: {step['search_string']}"
            )
        ...
        if isinstance(module, dict) and module.get("is_web_source", False):
            if (
                ...
                default_sources = _get_default_sources_for_module(
                    module.get("name", "")
                )
                step["source_selection"] = default_sources
                logger.info(
                    f"Tilføjet default sources for {module.get('name', '')}: {default_sources}"
                )
...
async def complete_recipe(raw_recipe: dict, goal: str = "") -> dict:
    ...
    for step in recipe.get("steps", []):
        ...
        if module_name:
            try:
                ...
                module_card = await module_validator.get_enhanced_module_card(
                    module_name
                )
                if module_card:
                    ...
                    step["module"][
                        "is_web_source"
                    ] = module_card.requires_source_selection
    ...
    # NEW: Validate filters against API (deprecated - now a no-op)
    logger.info("Trin 2.8: Validerer filtre mod API")
    # Recipe filter validation is now handled by enrich_recipe_with_api()
    logger.info(
        "validate_filters_against_api called (now handled by enrich_recipe_with_api, skipping)"
    )
    ...
    for idx, step in enumerate(recipe.get("steps", [])):
        ...
        step["educational"][
            "what_counts_as_hit"
        ] = f"**Hit types:** {', '.join(hit_def['hit_types'])}\n\n**Indicators:** {'; '.join(hit_def['indicators'])}"
    ...
    for i, step in enumerate(recipe.get("steps", [])):
        module = step.get("module", {})
        logger.info(
            f"Step {i+1}: module={module.get('name', 'Unknown')}, is_web_source={module.get('is_web_source', False)}, source_selection={step.get('source_selection', [])}"
        )
