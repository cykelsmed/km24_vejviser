...
def _get_default_sources_for_module(module_name: str) -> list[str]:
    ...
    if "lokalpolitik" in module_lower:
        ...
        return ["Miljøministeriet", "Beskæftigelsesministeriet", "Erhvervsministeriet", "Arbejdstilsynet", "Erhvervsstyrelsen"]
...
async def _enrich_with_module_specific_filters(step: dict, goal: str) -> dict:
    ...
    try:
        ...
        if module_card.requires_source_selection and (not step.get("source_selection") or len(step.get("source_selection", [])) == 0):
            default_sources = _get_default_sources_for_module(module_name)
            if default_sources:
                step["source_selection"] = default_sources
                logger.info(f"Tilføjet default kilder for {module_name}: {default_sources}")
    ...
    for filter_key in filters.keys():
        ...
        if filter_key in ["source_selection", "kilder", "sources"]:
            if "web_source" not in supported_types:
                to_remove.append(filter_key)
                logger.warning(f"Module {module_name} does not support source selection")
