...
class ModuleRef(BaseModel):
    ...
    is_web_source: bool = Field(default=False, description="Whether module requires source selection")
...
class Step(BaseModel):
    ...
    guardrails: Guardrails = Field(default_factory=Guardrails, description="Safety rules")
    source_selection: List[str] = Field(default_factory=list, description="Selected sources for web modules")
...
class SyntaxGuide(BaseModel):
    ...
    basic_syntax: List[str] = Field(default_factory=list, description="Basic search syntax examples")
    advanced_syntax: List[str] = Field(default_factory=list, description="Advanced search patterns")
    tips: List[str] = Field(default_factory=list, description="Search tips and tricks")

class Quality(BaseModel):
    """Quality assurance checks."""
    checks: List[str] = Field(default_factory=list, description="Quality checks performed")
...
class Artifacts(BaseModel):
    ...
    exports: List[Literal["csv", "json", "xlsx"]] = Field(default_factory=list, description="Export formats")
    reports: List[str] = Field(default_factory=list, description="Generated reports")
    visualizations: List[str] = Field(default_factory=list, description="Data visualizations")
...
class Overview(BaseModel):
    ...
    module_flow: List[str] = Field(default_factory=list, description="Module execution flow")
    estimated_duration: str = Field(default="1-2 weeks", description="Estimated investigation duration")

class Scope(BaseModel):
    """Investigation scope and boundaries."""
    primary_focus: str = Field(..., description="Primary investigation focus")
    secondary_areas: List[str] = Field(default_factory=list, description="Secondary investigation areas")
...
class HitBudget(BaseModel):
    ...
    budget_allocation: Dict[str, int] = Field(default_factory=dict, description="Budget per step")
    resource_requirements: List[str] = Field(default_factory=list, description="Resource needs")

class Notifications(BaseModel):
    """Notification configuration."""
    primary: Notif = Field(default="daily", description="Primary notification frequency")
    secondary: Optional[Notif] = Field(None, description="Secondary notification frequency")
...
class UseCaseResponse(BaseModel):
    ...
    parallel_profile: ParallelProfile = Field(..., description="Parallel execution profile")
    steps: List[Step] = Field(..., description="Investigation steps")
    cross_refs: List[CrossRef] = Field(default_factory=list, description="Cross-references")
    ...
    def validate_structure(self):
        ...
        if step_numbers and (min(step_numbers) != 1 or step_numbers != list(range(1, len(step_numbers) + 1))):
        ...
        for cross_ref in cross_refs:
            if cross_ref.from_step not in valid_step_numbers:
                raise ValueError(f"Cross-reference from_step {cross_ref.from_step} does not exist")
            if cross_ref.to_step not in valid_step_numbers:
                raise ValueError(f"Cross-reference to_step {cross_ref.to_step} does not exist")
