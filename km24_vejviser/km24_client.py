...
class KM24APIClient:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning(
                "KM24_API_KEY ikke sat - API funktionalitet vil være begrænset"
            )
    ...
    def _load_cache(self, cache_path: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            cache_time = datetime.fromisoformat(
                cached_data.get("cached_at", "1970-01-01")
            )
    ...
    async def _make_request(
        ...
        if not self.api_key:
            return KM24APIResponse(
                success=False, error="KM24_API_KEY ikke konfigureret"
            )
        ...
        try:
            ...
            if response.status_code in (404, 405):
                logger.error(
                    f"API fejl {response.status_code}: endpoint findes ikke eller metode ikke tilladt"
                )
                return KM24APIResponse(
                    success=False,
                    error=f"API fejl {response.status_code}: endpoint ikke fundet / metode ikke tilladt",
                )
            # Generic error
    ...
    async def get_modules_detailed(
    ...
    async def get_filter_options(
        self, module_slug: str, filter_type: str, force_refresh: bool = False
    ) -> KM24APIResponse:
        """Ikke dokumenteret i API. Returnerer fejl for at undgå falsk kontrakt."""
        return KM24APIResponse(
            success=False,
            error="Endpoint ikke dokumenteret: modules/{slug}/filters/{type}",
        )

    async def get_media_sources(
        self, media_type: str = "danish", force_refresh: bool = False
    ) -> KM24APIResponse:
    ...
    async def get_web_sources(
        ...
        return await self._make_request(
            f"/web-sources/categories/{module_id}", force_refresh
        )
    ...
    async def get_branch_codes_detailed(
        self, force_refresh: bool = False
    ) -> KM24APIResponse:
    ...
    async def get_filter_options_for_module(
        self, module_id: int, force_refresh: bool = False
    ) -> KM24APIResponse:
        """Ikke dokumenteret. Brug get_module_details + parts + generic-values/web-sources i stedet."""
        return KM24APIResponse(
            success=False,
            error="Endpoint ikke dokumenteret: modules/{id}/filter-options",
        )
