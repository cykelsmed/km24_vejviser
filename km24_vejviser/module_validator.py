...
import re
from .km24_client import get_km24_client
...
class ModuleValidator:
    ...
    async def _load_modules(self) -> bool:
        ...
        try:
            ...
            if result.success and result.data:
                ...
                self._module_titles = [
                    mod.get("title", "") for mod in self._modules_cache
                ]
                self._module_slugs = [
                    mod.get("slug", "") for mod in self._modules_cache
                ]
                self._module_id_by_title = {
                    mod.get("title", ""): int(mod.get("id"))
    ...
    async def _ensure_module_parts(self, module_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if details.success and details.data:
                ...
                return parts
            logger.warning(
                f"Kunne ikke hente parts for modul {module_id}: {details.error}"
            )
    ...
    async def get_module_parts_by_title(
        self, module_title: str
    ) -> List[Dict[str, Any]]:
    ...
    async def validate_filters_against_parts(
        self, module_title: str, filters: Dict[str, Any]
    ) -> List[str]:
        ...
        for key in list(filters.keys()):
            ...
            if mapped:
                if mapped not in part_types:
                    warnings.append(
                        f"Modul '{module_title}' understøtter ikke filter '{key}' ({mapped})"
                    )
                continue
            ...
            if key.lower() not in {"periode", "hitlogik", "region"}:
                warnings.append(
                    f"Filter-nøgle '{key}' matcher ingen kendt part for modulet '{module_title}'"
                )
        ...
        if has_web_source and not filters.get("source_selection"):
            ...
            warnings.append(
                f"Webkilde-modul '{module_title}' kræver 'source_selection' (kildevalg)"
            )
    ...
    def _generate_match_reason(
        self, query: str, title: str, slug: str, similarity: float
    ) -> str:
    ...
    async def validate_recommended_modules(
    ...
    async def get_module_suggestions_for_goal(
        ...
        sorted_matches = sorted(
            unique_matches.values(), key=lambda x: x.confidence, reverse=True
        )
    ...
    async def get_enhanced_module_card(
        self, module_title: str
    ) -> Optional[EnhancedModuleCard]:
        ...
        for module in self._modules_cache:
            if module.get("title") == module_title:
                ...
                logger.info(
                    f"Module data: {module.get('longDescription', 'NO LONG DESC')[:100]}..."
                )
                ...
                for part in module.get("parts", []):
                    filter_info = {
                        ...
                        "practical_use": self._get_practical_filter_use(
                            part.get("part"), part.get("name")
                        ),
                    }
                ...
                complexity_level = self._calculate_complexity_level(
                    total_filters, requires_source
                )

                # Extract data frequency from description
                data_freq = self._extract_data_frequency(
                    module.get("longDescription", "")
                )

                logger.info(
                    f"Module stats - Filters: {total_filters}, Complexity: {complexity_level}"
                )
    ...
    def _calculate_complexity_level(
        self, total_filters: int, requires_source: bool
    ) -> str:
    ...
    async def get_filter_recommendations(
        self, module_title: str, goal: str = ""
        ...
        sorted_filters = sorted(
            card.available_filters, key=lambda x: priority_map.get(x["type"], 9)
        )
    ...
    async def analyze_complexity(
        ...
        if "industry" in filters and filters["industry"]:
            ...
            if any(f["type"] == "industry" for f in card.available_filters):
                optimizations.append(
                    "Tilføj branche-filter for færre og mere relevante hits"
                )
                complexity_score += 50
        ...
        if "municipality" in filters and filters["municipality"]:
            ...
            if any(f["type"] == "municipality" for f in card.available_filters):
                optimizations.append(
                    "Begræns geografisk for mere fokuseret overvågning"
                )
                complexity_score += 30
        ...
        if any(f["type"] == "amount_selection" for f in card.available_filters):
            if "amount" not in filters:
                optimizations.append(
                    "Overvej beløbsgrænse for at fokusere på større sager"
                )
                complexity_score += 20
        ...
        notification_rec = {
            ...
            "reason": reason,
            "optimization": (
                optimizations[0] if optimizations else "Konfiguration ser god ud"
            ),
        }

        return ComplexityAnalysis(
            estimated, efficiency, notification_rec, optimizations
        )
    ...
    async def get_cross_module_intelligence(
        self, modules: List[str]
    ) -> List[Dict[str, Any]]:
        ...
        for workflow in workflows:
            if workflow["primary"] in modules:
                relevant_connections = [
                    mod for mod in workflow["connects_to"] if mod in modules
                ]
